...
cursor.execute('insert into temps values("12/2/2011",36)')
cursor.execute('insert into temps values("12/3/2011",42)')
...
cursor.execute("delete from temps where temp = 41")
cursor.execute("select * from temps")
rows = cursor.fetchall()
for row in rows:
	print("%s %s" % (row[0], row[1]))
